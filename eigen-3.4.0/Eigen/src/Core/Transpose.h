 ...
 * m = m.transpose().eval();
 * \endcode
 * and is faster and also safer because in the latter line of code, forgetting
 * the eval() results in a bug caused by \ref TopicAliasing "aliasing".
 *
 ...
 * \endcode
 * and is faster and also safer because in the latter line of code, forgetting
 * the eval() results in a bug caused by aliasing.
 *
 * Notice however that this method is only useful if you want to replace a
