 ...
 * reductions
 * \tparam Direction indicates whether to operate on columns (#Vertical) or rows
 * (#Horizontal)
 *
 ...
 * then re-assemble the outputs in a matrix expression:
...
public:
   ...
   * (resp. column) of the nested expression
   * \sa begin(), cend()
   */
  iterator end() {
   ...
   * are normalized. The referenced matrix is \b not modified.
