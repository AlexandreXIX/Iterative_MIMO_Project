 ...
 * blocking size along the same dimension.
 * \param[in,out] m Input: the number of rows of the left hand side. Output: the
 * blocking size along the same dimension.
 * \param[in,out] n Input: the number of columns of the right hand side. Output:
 * the blocking size along the same dimension.
 *
 * Given a m x k times k x n matrix product of scalar types \c LhsScalar and \c
 * RhsScalar, this function computes the blocking size parameters along the
...
public:
  ...
  EIGEN_STRONG_INLINE void loadRhsQuad(const RhsScalar *b,
                                       RhsPacket &dest) const {
    loadRhsQuad_impl(b, dest, typename conditional < RhsPacketSize == 16,
 ...
 * estimate the ideal blocking size parameters.
 * \sa setCpuCacheSize */
