 ...
 *rotation().
 * \tparam _Options has the same meaning as in class Matrix. It allows to
 *specify DontAlign and/or RowMajor. These Options are passed directly to the
...
public:
   ...
   * Mode!=Projective), then \a other can also be:
   * \li a point of size Dim (computes: \code this->linear() * other +
   * this->translation()\endcode),
   * \li a set of N points as a Dim x N matrix (computes: \code (this->linear()
   ...
   * explicitly apply the linear part only:
   * \code
   * Affine3f A;
   * Vector3f v1, v2;
   * v2 = A.linear() * v1;
 ...
 * other to \c *this and returns a reference to \c *this.
 * \sa pretranslate()
 */
template <typename Scalar, int Dim, int Mode, int Options>
template <typename OtherDerived>
