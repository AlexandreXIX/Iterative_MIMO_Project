...
QAMConstellation::QAMConstellation(const ProblemParameters* params) : Constellation(params->GetN(), std::vector<int>(params->GetN())), params(params) {
...
const ProblemParameters* QAMConstellation::GetParameters() const { return params; };

// Checks if a given pointer is the same pointer as this object's parameters (output True means same)
bool QAMConstellation::SameParameters(const ProblemParameters* otherPointer) const {return params == otherPointer; };

// Helper function
// Finds the corresponding coordinates of the QAM symbol for a given message (only takes real)
